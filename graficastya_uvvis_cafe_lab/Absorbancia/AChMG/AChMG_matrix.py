...
# %% [markdown]
# ## unificar datos de absorbancia – AChMG
#
# En esta celda convertimos todos los archivos `.txt` con datos de absorbancia de muestras de café de Chiapas - AChMG (molienda grande) a una sola matriz `.csv`, lista para analizar.
#
...
# - Ordenarlos por gramaje (extraído del nombre del archivo, como AChMG05, AChMG20, etc.)
...
#
# El resultado se guarda en un archivo llamado `AChMG_matrix.csv`
...
df_consolidado.to_csv('AChMG_matrix.csv', index=False)

print("Archivo 'AChMG_matrix.csv' creado con éxito")

# %% [markdown]
# ## verificamos la matriz de datos
#
# Una vez que ya unimos todos los datos en el archivo `AChMG_matrix.csv`, ahora toca revisarlos para entender su forma.
#
...
# %%
import pandas as pd
datos=pd.read_csv("AChMG_matrix.csv")
...
# - Algunas muestras tienen más datos que otras (por ejemplo, AChMG05 tiene 405 valores válidos, mientras que AChMG50  tiene 372).

# %% [markdown]
# ## Graficamos las curvas de absorbancia ordenadas por intensidad
#
# Ya con la matriz de datos limpia (valores NaN donde hubo saturación), graficamos las curvas de absorbancia de las muestras de café de Chiapas
#
# Pasos:
...
df = pd.read_csv("AChMG_matrix.csv")
...
plt.title('Curvas de absorbancia - Café de Chiapas MG (Molienda grande)', fontsize=14)
...
fig.update_layout(
    title="Curvas de absorbancias - Café de Chiapas MG (Molienda grande)",
...
<body>
  ...
  <h1>Reporte de Absorbancia UV-Vis - Café de Chiapas MG (Molienda grande)</h1>
