...
# %% [markdown]
# ## unificar datos de absorbancia – AChMN
#
# En esta celda convertimos todos los archivos `.txt` con datos de absorbancia de muestras de café de Chiapas - AChMN (molienda normal) a una sola matriz `.csv`, lista para analizar.
#
...
# - Ordenarlos por gramaje (extraído del nombre del archivo, como AChMN05, AChMN20, etc.)
...
#
# El resultado se guarda en un archivo llamado `AChMN_matrix.csv`
...
df_consolidado.to_csv('AChMN_matrix.csv', index=False)

print("Archivo 'AChMN_matrix.csv' creado con éxito")

# %% [markdown]
# ## verificamos la matriz de datos
#
# Una vez que ya unimos todos los datos en el archivo `AChMN_matrix.csv`, ahora toca revisarlos para entender su forma.
#
...
# %%
import pandas as pd
datos=pd.read_csv("AChMN_matrix.csv")
...
# - Algunas muestras tienen más datos que otras (por ejemplo, AChMN05 tiene 394 valores válidos, mientras que AChMN50  tiene 367).
...
df = pd.read_csv("AChMN_matrix.csv")
...
plt.title('Curvas de absorbancia - Café de Chiapas MN (Molienda normal)', fontsize=14)
...
fig.update_layout(
    title="Curvas de absorbancias - Café de Chiapas MN (Molienda normal)",
...
<body>
  ...
  <h1>Reporte de Absorbancia UV-Vis - Café de Chiapas MN (Molienda normal)</h1>
