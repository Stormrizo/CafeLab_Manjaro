...
# %% [markdown]
# ## unificar datos de absorbancia – AMiMG
#
# En esta celda convertimos todos los archivos `.txt` con datos de absorbancia de muestras de café de Mixteca Alta Oaxaqueña - AMiMG (molienda grande) a una sola matriz `.csv`, lista para analizar.
#
...
# - Ordenarlos por gramaje (extraído del nombre del archivo, como AMiMG05, AMiMG20, etc.)
...
#
# El resultado se guarda en un archivo llamado `AMiMG_matrix.csv`
...
df_consolidado.to_csv('AMiMG_matrix.csv', index=False)

print("Archivo 'AMiMG_matrix.csv' creado con éxito")

# %% [markdown]
# ## verificamos la matriz de datos
#
# Una vez que ya unimos todos los datos en el archivo `AMiMG_matrix.csv`, ahora toca revisarlos para entender su forma.
#
...
# %%
import pandas as pd
datos=pd.read_csv("AMiMG_matrix.csv")
...
# - Algunas muestras tienen más datos que otras (por ejemplo, AMiMG05 tiene 405 valores válidos, mientras que AMiMG50  tiene 381).
...
df = pd.read_csv("AMiMG_matrix.csv")
...
plt.title('Curvas de absorbancia - Café de Mixteca Alta Oxaqueña MG (Molienda grande)', fontsize=14)
...
plt.xlabel('Longitud de onda (nm)', fontsize=12)
plt.ylabel('Absorbancia', fontsize=12)
plt.title('Curvas de absorbancia - Café de Mixteca Alta Oaxaqueña MG (Molienda grande)', fontsize=14)
...
fig.update_layout(
    title="Curvas de absorbancia - Café de Mixteca Alta Oaxaqueña (Molienda grande)",
...
<body>
  ...
  <h1>Reporte de Absorbancia UV-Vis - Café de Mixteca Alta Oaxaqueña molienda grande (MG)</h1>
